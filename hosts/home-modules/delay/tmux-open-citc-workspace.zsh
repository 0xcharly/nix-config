...
# Open CITC workspaces.
tmux-open-citc-workspace() {
  ...
  local cmd="${TMUX_OPEN_CITC_WORKSPACE_COMMAND:-"command hg citc --list 2> /dev/null"}"
  local workspace="$(eval "$cmd" |
  ...
  if ! tmux has-session -t "$workspace" 2>/dev/null; then
    ...
    tmux new-session -ds "$workspace" -c "$(hg hgd -qf "$workspace" 2> /dev/null)"
  fi
  ...
  unset workspace # Ensure this doesn't end up appearing in prompt expansion.
  zle reset-prompt
  return $ret
}

zle     -N            tmux-open-citc-workspace
bindkey -M emacs '^F' tmux-open-citc-workspace
bindkey -M vicmd '^F' tmux-open-citc-workspace
bindkey -M viins '^F' tmux-open-citc-workspace

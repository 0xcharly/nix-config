...
Singleton {
    ...
    component Monitor: QtObject {
        ...
        readonly property bool isAppleDisplay: modelData.model.startsWith("ProDisplayXDR")
            ...
            stdout: StdioCollector {
                onStreamFinished: {
                    if (monitor.isAppleDisplay) {
                        const val = parseInt(text.trim().slice(0, -1));
        ...
        function setBrightness(value: real): void {
            ...
            if (isAppleDisplay)
                Quickshell.execDetached(["apdbctl", "set", `${rounded}%`]);
            else if (isDdc)

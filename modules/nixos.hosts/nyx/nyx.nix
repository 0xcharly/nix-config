      ...
      { config, ... }:
      {
        ...
        node = {
          boot.initrd.hoopsnake = {
            ...
            # Secrets are registered under the tailnet machine name (term-nyx),
            # not networking.hostName (nyx) that the module defaults derive.
            clientIdFile = config.age.secrets."services/hoopsnake/term-nyx/tailscale-client-id".path;
            clientSecretFile = config.age.secrets."services/hoopsnake/term-nyx/tailscale-client-secret".path;
            privateHostKeyFile = config.age.secrets."services/hoopsnake/term-nyx/ssh_host_ed25519_key".path;
          };

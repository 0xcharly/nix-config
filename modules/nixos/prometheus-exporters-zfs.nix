{ self, ... }:
{
  flake.nixosModules.prometheus-exporters-zfs =
    ...
    {
      options.node.services.prometheus.report-zfs-scrub = with lib; {
        enable = mkEnableOption "Write ZFS scrub metrics for the node exporter textfile collector";
      };

      config =
        let
          cfg = config.node.services.prometheus.report-zfs-scrub;
        in
        {
          ...
          # module).
          systemd.services.prometheus-zfs-scrub-textfile = lib.mkIf cfg.enable {
          ...
          systemd.timers.prometheus-zfs-scrub-textfile = lib.mkIf cfg.enable {

{lib, ...}: {
  ...
  programs.ssh.knownHosts = let
    ...
    genKnownHostAttrs = host: type: key: {
      "${host}/${type}" = {
        hostNames = lib.singleton host;
        publicKey = lib.concatStringsSep " " [type key];
      };
    };
    genKnownHostAttrsList = lib.flatten (lib.mapAttrsToList (
        host: keys:
          lib.mapAttrsToList (type: key: genKnownHostAttrs host type key) keys
      )
      hosts);
  in
    lib.mergeAttrsList genKnownHostAttrsList;
}
